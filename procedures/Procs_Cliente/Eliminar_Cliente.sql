...
CREATE PROC Procs_ELIMINAR_CLIENTE
	@CodCl INT,
...
BEGIN
	...
	BEGIN TRY
		...
		-------VERIFICAR SI EL CLINETE YA ESTA INACTIVO
		IF (@Cliente_Existe = 0)
		BEGIN
			ROLLBACK TRANSACTION;
			SET @MENSAJE = 'El cliente ya esta inactivo';
			RETURN;	
		END
		...
		COMMIT TRANSACTION;
		SET @MENSAJE = 'El cliente se elimino correctamente';
	END TRY
	BEGIN CATCH
		IF @@TRANCOUNT > 0
			ROLLBACK TRANSACTION;

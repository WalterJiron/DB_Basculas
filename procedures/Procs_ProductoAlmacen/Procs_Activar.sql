USE Bascula

GO

CREATE PROC Procs_Activar_ProductoAlmacen
    @CodAlmacen INT,
    @CodProd INT,
    @MENSAJE NVARCHAR(100) OUTPUT
AS
BEGIN
    SET NOCOUNT ON;

    BEGIN TRY
        -- Validar que no sean nulos
        IF @CodAlmacen IS NULL OR @CodProd IS NULL
        BEGIN
            SET @MENSAJE = 'Los campos codigo de Almacen y codigo de Producto son obligatorios.';
            RETURN;
        END

        BEGIN TRANSACTION;

        -- Verificar existencia
        DECLARE @Existe BIT;
        SET @Existe = (SELECT EstadoAlmacenProd FROM ProductoAlmacen WITH(UPDLOCK, ROWLOCK)WHERE CodAlmacen = @CodAlmacen AND CodProd = @CodProd);

        IF @Existe IS NULL
        BEGIN
            ROLLBACK TRANSACTION;
            SET @MENSAJE = 'El registro no existe en ProductoAlmacen.';
            RETURN;
        END

        IF @Existe = 1
        BEGIN
            ROLLBACK TRANSACTION;
            SET @MENSAJE = 'El producto ya esta activo en este almacen.';
            RETURN;
        END


        UPDATE ProductoAlmacen SET 
			EstadoAlmacenProd = 1,
			DateDelete = NULL
        WHERE CodAlmacen = @CodAlmacen AND CodProd = @CodProd;

        IF @@ROWCOUNT <> 1
        BEGIN
            ROLLBACK TRANSACTION;
            SET @MENSAJE = 'No se pudo activar el producto del almacen.';
            RETURN;
        END

        COMMIT TRANSACTION;
        SET @MENSAJE = 'Producto activado correctamente del almacen.';
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0
            ROLLBACK TRANSACTION;

        SET @MENSAJE = 'Error al activar producto del almacen: ' + ERROR_MESSAGE();
    END CATCH
END;
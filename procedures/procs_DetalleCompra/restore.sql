...
CREATE PROC ProcRestoreDetalleCompra
@CodDetCompra INT,
@CodCompra INT,
...
BEGIN
	...
	BEGIN TRY
		...
		IF @exist_DetCompra = 1
		BEGIN
			SET @Mensaje = 'ERROR: El detalle de compra del código ('+ TRY_CONVERT(nvarchar(10),@CodDetCompra) +') ya está activo.';
			RETURN;
		END
		...
		SET @Mensaje = 'Detalle de compra reactivado correctamente.';
	END TRY
	BEGIN CATCH			
		SET @Mensaje = 'Error al reactivar detalle de compra: ' + ERROR_MESSAGE();
	END CATCH
END
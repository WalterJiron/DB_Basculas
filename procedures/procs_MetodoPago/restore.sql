...
CREATE PROC ProcRestoreMetodoPago
@CodMetodoPago INT,
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			ROLLBACK TRANSACTION;
			SET @Mensaje = 'ERROR: El método de pago del código ('+ TRY_CONVERT(nvarchar(10),@CodMetodoPago) +') ya está activo';
			RETURN;
		END
		...
		BEGIN
			ROLLBACK TRANSACTION;
			SET @Mensaje = 'Error inesperado al restaurar el método de pago.';
			RETURN;
		END
		
		COMMIT TRANSACTION;
		SET @Mensaje = 'Método de pago restaurado correctamente.';
	END TRY
	BEGIN CATCH
		IF @@TRANCOUNT > 0
			ROLLBACK TRANSACTION;
			
		SET @Mensaje = 'Error al restaurar el método de pago: ' + ERROR_MESSAGE();
	END CATCH
END